...
```
/spec-writer <ticket-id>
```

The ticket must already exist at `.claude/swe-state/{ticket-id}/ticket.json` (produced by Stage 1 `/ticket-intake`).
...
1. **Ticket file**: `.claude/swe-state/{ticket-id}/ticket.json` must exist
   - If missing, tell the user: "No ticket found. Run `/ticket-intake {ticket-id}` first."
...
```
/spec-review .claude/specs/{ticket-id}.md
```
...
If the user updates the spec, re-run `/spec-review` on the updated version.
...
```
/spec-review .claude/specs/{ticket-id}-impl.md
```
...
- **Missing ticket file**: Direct user to run `/ticket-intake {ticket-id}` first
